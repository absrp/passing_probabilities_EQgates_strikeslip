...
def build_regression_double_bend_length(grouped,groupid,stress_typeYN,feature_type,axesid,minx,maxx,xlabel,ptsize):
    ...
    palette = {'breached': 'teal', 'unbreached': 'darkorange'}
    if max(xfeature)>90:
        sns.swarmplot(data=group,x=10**feature_type,y='Breached or unbreached',ax=axesid,size=ptsize,hue="Breached or unbreached",palette=palette,alpha=0.7)
    else:
...
def build_cdf(grouped,groupid, stress_typeYN, length_or_angle, colorB,colorU, axesid,xlabel,labelB, labelU):
    ...
    axesid.grid(color='lightgray', linewidth=0.5)
    axesid.legend()
