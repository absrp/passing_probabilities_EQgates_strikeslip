function  [x,y,utmzonei,utmhemi] = wgs2utm(Lat,Lon,utmzonei,utmhemi)
% -------------------------------------------------------------------------
% [x,y,utmzonei] = wgs2utm(Lat,Lon,Zone)
%
% Description:
...
%    utmzonei (optional): UTM longitudinal zone. Scalar or same size as Lat
%       and Lon.
...
%    utmzonei: UTM longitudinal zone.
...
%    [x1,y1,utmzonei1,utmhemi1] = wgs2utm(Lat,Lon)
%
...
%    [x2,y2,utmzonei2,utmhemi2] = wgs2utm(Lat,Lon,60,'S')
%
%    % forcing different UTM zone and hemisphere for each data entry
%    % note: resulting easting and northing are way off the usual values
%    utmzonei = floor(59.*rand(3))+1
%    utmhemi = char(78 + 5.*round(rand(3)))
%    [x3,y3,utmzonei3,utmhemi3] = wgs2utm(Lat,Lon,utmzonei,utmhemi)
%
% Author: 
%   Alexandre Schimel
...
if exist('utmzonei','var') && exist('utmhemi','var')
    n3=size(utmzonei);
    n4=size(utmhemi);
    if (sort(n3)~=sort(n4))
        error('utmzonei and utmhemi should have same size');return
    end
    if max(n3)~=1 && max(n3)~=max(n1)
        error('utmzonei should have either same size as Lat and Long, or size=1');return
    end
end
% expand utmzonei and utmhemi if needed
if exist('utmzonei','var') && exist('utmhemi','var')
    n3=size(utmzonei);
    n4=size(utmhemi);
    if n3==[1 1]
        utmzonei = utmzonei.*ones(size(Lat));
...
if exist('utmzonei','var')
    Lon0 = 6.*utmzonei-183; % reference longitude in degrees
else
